...
async def connect(connection_id: str, environ: dict) -> None:
    try:
        ...
        # Add client to conversation room for browser tab switching
        await sio.enter_room(connection_id, f'conversation_{conversation_id}')
        logger.info(f'Added connection {connection_id} to room conversation_{conversation_id}')
