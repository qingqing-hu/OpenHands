...
class ActionExecutor:
    ...
    async def ainit(self):
        ...
        logger.info('[DEBUG] ActionExecutor.ainit: 开始初始化bash会话')
        self.bash_session = self._create_bash_session()
        logger.info('[DEBUG] ActionExecutor.ainit: Bash会话初始化完成')

        # Start browser initialization in the background
        logger.info('[DEBUG] ActionExecutor.ainit: 开始后台初始化浏览器')
        self.browser_init_task = asyncio.create_task(self._init_browser_async())
        logger.info('[DEBUG] ActionExecutor.ainit: 浏览器初始化任务已启动')

        plugin_timeout = int(os.environ.get('INIT_PLUGIN_TIMEOUT', '120'))
        logger.info(f'[DEBUG] ActionExecutor.ainit: 开始初始化插件，超时时间: {plugin_timeout}秒')
        logger.info(f'[DEBUG] ActionExecutor.ainit: 待初始化的插件: {[plugin.__class__.__name__ for plugin in self.plugins_to_load]}')
        
        await wait_all(
            (self._init_plugin(plugin) for plugin in self.plugins_to_load),
            timeout=plugin_timeout,
        )
        logger.info('[DEBUG] ActionExecutor.ainit: 所有插件初始化完成')
    ...
    async def _init_plugin(self, plugin: Plugin):
        logger.info(f'[DEBUG] ActionExecutor._init_plugin: 开始初始化插件: {plugin.name} ({plugin.__class__.__name__})')
        ...
        try:
            ...
            logger.error(f'[DEBUG] ActionExecutor._init_plugin: 插件初始化失败: {plugin.name} - 错误: {e}')
            import traceback
            logger.error(f'[DEBUG] ActionExecutor._init_plugin: 详细错误信息: {traceback.format_exc()}')
            raise
...
if __name__ == '__main__':
    ...
    async def lifespan(app: FastAPI):
        ...
        logger.info('[DEBUG] ActionExecutor: 开始初始化ActionExecutor...')
        logger.info(f'[DEBUG] ActionExecutor: 插件数量: {len(plugins_to_load)}')
        logger.info(f'[DEBUG] ActionExecutor: 插件列表: {[plugin.__class__.__name__ for plugin in plugins_to_load]}')
        logger.info(f'[DEBUG] ActionExecutor: 工作目录: {args.working_dir}')
        logger.info(f'[DEBUG] ActionExecutor: 用户名: {args.username}')
        ...
        await client.ainit()
        logger.info('[DEBUG] ActionExecutor: 异步初始化完成')
