...
<<<<<<< /Users/yddyf/Documents/code/openhands8/OpenHands-main-3/openhands/llm/llm.py
...
||||||| /tmp/source-analysis/OpenHands-main/openhands/llm/llm.py
=======
# cache prompt supporting models
# remove this when we gemini and deepseek are supported
CACHE_PROMPT_SUPPORTED_MODELS = [
...
>>>>>>> /tmp/colleague-analysis/colleague-code/openhands/llm/llm.py
...
class LLM(RetryMixin, DebugMixin):
    ...
    def __init__(
        ...
        """
        logger.info(f'[DEBUG] LLM初始化开始 - 模型: {config.model}')
        logger.info(f'[DEBUG] LLM配置 - base_url: {config.base_url}')
        logger.info(f'[DEBUG] LLM配置 - use_token_auth: {getattr(config, "use_token_auth", False)}')
        logger.info(f'[DEBUG] LLM配置 - response_wrapper_field: {getattr(config, "response_wrapper_field", None)}')
...
>>>>>>> /tmp/colleague-analysis/colleague-code/openhands/llm/llm.py
        ...
        # Check if we need to use custom proxy
        if self._is_custom_proxy():
            logger.info('[DEBUG] LLM: 启用自定义代理模式用于内网部署')
            logger.info(f'[DEBUG] LLM: Token认证模式: {self.config.use_token_auth}')
            logger.info(f'[DEBUG] LLM: 代理URL: {self.config.base_url}')
            self._setup_custom_proxy()
        else:
            logger.info('[DEBUG] LLM: 使用标准模式，未启用自定义代理')
    ...
    def init_model_info(self) -> None:
        if self._tried_model_info:
            logger.info('[DEBUG] init_model_info: 已经尝试过获取模型信息，跳过')
            return
        self._tried_model_info = True
        
        logger.info(f'[DEBUG] init_model_info: 开始初始化模型信息 - 模型: {self.config.model}')
        
        # Skip model info retrieval for custom proxy mode to avoid network timeouts
        if self._is_custom_proxy():
            logger.info('[DEBUG] init_model_info: 检测到自定义代理模式，跳过模型信息获取以避免网络超时')
            # Initialize required attributes for custom proxy mode using same logic as normal flow
            ...
            logger.info(f'[DEBUG] init_model_info: 自定义代理模式 - function_calling_active: {self._function_calling_active}')
            return
    ...
    def _is_custom_proxy(self) -> bool:
        """Check if we should use custom proxy mode.
        
        Returns True if use_token_auth is enabled, indicating this is a custom proxy setup.
        """
        result = self.config.use_token_auth
        logger.info(f'[DEBUG] _is_custom_proxy: use_token_auth = {self.config.use_token_auth}, 返回值 = {result}')
        logger.info(f'[DEBUG] _is_custom_proxy: config对象类型 = {type(self.config)}')
        logger.info(f'[DEBUG] _is_custom_proxy: config所有属性 = {[attr for attr in dir(self.config) if not attr.startswith("_")]}')
        return result

    def _setup_custom_proxy(self) -> None:
        """Setup custom proxy handling for internal network deployments."""
        # Store original completion function
        original_completion = self._completion_unwrapped
        
        def custom_proxy_wrapper(*args: Any, **kwargs: Any) -> Any:
            """Custom wrapper for proxy requests with token auth and response unwrapping."""
            
            # Use httpx to make custom request
            return self._make_custom_proxy_request(*args, **kwargs)
        
        # Replace the completion function
        self._completion_unwrapped = custom_proxy_wrapper

    def _make_custom_proxy_request(self, *args: Any, **kwargs: Any) -> Any:
        """Make a custom proxy request with token authentication."""
        
        logger.info('[DEBUG] _make_custom_proxy_request: 开始处理自定义代理请求')
        logger.info(f'[DEBUG] _make_custom_proxy_request: args数量: {len(args)}')
        logger.info(f'[DEBUG] _make_custom_proxy_request: kwargs键: {list(kwargs.keys())}')
        ...
        logger.info(f'[DEBUG] _make_custom_proxy_request: 消息数量: {len(messages)}')
        logger.info(f'[DEBUG] _make_custom_proxy_request: 第一条消息: {messages[0] if messages else "None"}')
        ...
        if self.config.api_key:
            ...
            logger.info(f'[DEBUG] _make_custom_proxy_request: 使用Token认证，Token长度: {len(self.config.api_key.get_secret_value())}')
        else:
            logger.warning('[DEBUG] _make_custom_proxy_request: 未设置API密钥')
        ...
        if self.config.proxy_headers:
            ...
            logger.info(f'[DEBUG] _make_custom_proxy_request: 添加了自定义代理头部: {list(self.config.proxy_headers.keys())}')
        
        logger.info(f'[DEBUG] _make_custom_proxy_request: 请求URL: {self.config.base_url}')
        logger.info(f'[DEBUG] _make_custom_proxy_request: 请求头部: {list(headers.keys())}')
        logger.info(f'[DEBUG] _make_custom_proxy_request: 请求数据: {request_data}')
        logger.info(f'[DEBUG] _make_custom_proxy_request: 请求超时设置: {self.config.timeout or 120}秒')
        logger.info(f'[DEBUG] _make_custom_proxy_request: TraceID: {trace_id}')
        
        # Make the request
        try:
            logger.info('[DEBUG] _make_custom_proxy_request: 开始发送HTTP请求')
            with httpx.Client(timeout=self.config.timeout or 120) as client:
                ...
                logger.info(f'[DEBUG] _make_custom_proxy_request: 收到HTTP响应，状态码: {response.status_code}')
                response.raise_for_status()
                
            response_data = response.json()
            logger.info(f'[DEBUG] _make_custom_proxy_request: 原始响应数据键: {list(response_data.keys())}')
            
            # Handle response wrapper field (e.g., extract from "data" field)
            if self.config.response_wrapper_field:
                logger.info(f'[DEBUG] _make_custom_proxy_request: 使用配置的响应包装字段: {self.config.response_wrapper_field}')
                if self.config.response_wrapper_field in response_data:
                    actual_response = response_data[self.config.response_wrapper_field]
                    logger.info(f'[DEBUG] _make_custom_proxy_request: 成功提取包装字段数据，键: {list(actual_response.keys()) if isinstance(actual_response, dict) else "非字典类型"}')
                else:
                    logger.warning(f'[DEBUG] _make_custom_proxy_request: 响应包装字段 "{self.config.response_wrapper_field}" 未在响应中找到')
                    actual_response = response_data
            else:
                logger.info('[DEBUG] _make_custom_proxy_request: 未配置响应包装字段，尝试自动检测')
                # Auto-detect "data" wrapper for backward compatibility
                if 'data' in response_data and 'choices' in response_data['data']:
                    logger.info('[DEBUG] _make_custom_proxy_request: 自动检测到data包装字段')
                    actual_response = response_data['data']
                else:
                    logger.info('[DEBUG] _make_custom_proxy_request: 未检测到data包装，使用原始响应')
                    actual_response = response_data
            
            logger.info(f'[DEBUG] _make_custom_proxy_request: 最终处理的响应数据键: {list(actual_response.keys()) if isinstance(actual_response, dict) else "非字典类型"}')
            
            # Convert to ModelResponse format
            logger.info('[DEBUG] _make_custom_proxy_request: 开始转换响应为ModelResponse格式')
            result = self._convert_proxy_response_to_model_response(actual_response)
            logger.info('[DEBUG] _make_custom_proxy_request: 响应转换完成')
            return result
            
        except httpx.TimeoutException as e:
            logger.error(f'[DEBUG] _make_custom_proxy_request: HTTP请求超时: {e}')
            logger.error(f'[DEBUG] _make_custom_proxy_request: 超时URL: {self.config.base_url}')
            logger.error(f'[DEBUG] _make_custom_proxy_request: 超时时长: {self.config.timeout or 120}秒')
            raise
        except httpx.RequestError as e:
            logger.error(f'[DEBUG] _make_custom_proxy_request: HTTP请求失败: {e}')
            logger.error(f'[DEBUG] _make_custom_proxy_request: 失败URL: {self.config.base_url}')
            raise
        except Exception as e:
            logger.error(f'[DEBUG] _make_custom_proxy_request: 自定义代理处理失败: {e}')
            import traceback
            logger.error(f'[DEBUG] _make_custom_proxy_request: 详细错误信息: {traceback.format_exc()}')
            raise

    def _convert_proxy_response_to_model_response(self, response_data: dict) -> Any:
        """Convert custom proxy response to standard ModelResponse format."""
        ...
        model_response = ModelResponse(
            id=response_data.get('id', f'custom-proxy-{int(time.time())}'),
