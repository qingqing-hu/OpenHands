...
class AgentSession:
    ...
    async def _create_runtime(
        ...
        self.logger.info(f'[DEBUG] AgentSession._create_runtime: 开始初始化runtime `{runtime_name}`...')
        self.logger.info(f'[DEBUG] AgentSession._create_runtime: 会话ID: {self.sid}')
        self.logger.info(f'[DEBUG] AgentSession._create_runtime: 用户ID: {self.user_id}')
        runtime_cls = get_runtime_cls(runtime_name)
        self.logger.info(f'[DEBUG] AgentSession._create_runtime: Runtime类: {runtime_cls.__name__}')
        ...
        try:
            self.logger.info('[DEBUG] AgentSession._create_runtime: 开始连接到runtime...')
            await self.runtime.connect()
            self.logger.info('[DEBUG] AgentSession._create_runtime: Runtime连接成功')
        except AgentRuntimeUnavailableError as e:
            self.logger.error(f'[DEBUG] AgentSession._create_runtime: Runtime连接失败: {e}')
