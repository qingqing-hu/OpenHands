...
<<<<<<< /Users/yddyf/Documents/code/openhands8/OpenHands-main-3/openhands/server/session/session.py
from openhands.llm.llm import LLM
||||||| /tmp/source-analysis/OpenHands-main/openhands/server/session/session.py
from openhands.llm.llm_registry import LLMRegistry
=======
from openhands.experiments.experiment_manager import ExperimentManagerImpl
from openhands.llm.llm import LLM
>>>>>>> /tmp/colleague-analysis/colleague-code/openhands/server/session/session.py
...
class Session:
    ...
    async def initialize_agent(
        ...
        # Store user settings for later use in LLM config
        self._user_settings = settings
        self.logger.info(f'[DEBUG] Session.initialize_agent: 保存用户设置 - llm_use_token_auth: {getattr(settings, "llm_use_token_auth", False)}')
...
>>>>>>> /tmp/colleague-analysis/colleague-code/openhands/server/session/session.py
        ...
        try:
            self.logger.info('[DEBUG] Session: 开始启动agent_session')
            self.logger.info(f'[DEBUG] Session: runtime_name: {self.config.runtime}')
            self.logger.info(f'[DEBUG] Session: agent: {agent}')
            self.logger.info(f'[DEBUG] Session: max_iterations: {max_iterations}')
            ...
            self.logger.info('[DEBUG] Session: agent_session启动成功')
...
<<<<<<< /Users/yddyf/Documents/code/openhands8/OpenHands-main-3/openhands/server/session/session.py
    def _create_llm(self, agent_cls: str | None) -> LLM:
        ...
        llm_config = self.config.get_llm_config_from_agent(agent_name)
        self.logger.info(f'[DEBUG] Session._create_llm: 开始创建LLM - agent_name: {agent_name}')
        self.logger.info(f'[DEBUG] Session._create_llm: 原始LLM配置 - 模型: {llm_config.model}')
        self.logger.info(f'[DEBUG] Session._create_llm: 原始LLM配置 - base_url: {llm_config.base_url}')
