...
class LLMConfig(BaseModel):
    ...
    Attributes:
        ...
        disable_stop_word: Disable stop word processing.
        ...
        use_token_auth: Use token authentication instead of API key (for custom proxy).
        proxy_headers: Additional headers for custom proxy requests.
        response_wrapper_field: Field name that wraps the actual response (e.g., "data" for internal proxies).
    """
    ...
    # total wait time: 8 + 16 + 32 + 64 = 120 seconds (源码版本的优化重试策略)
    ...
    disable_stop_word: bool | None = Field(default=False)  # 源码版本新功能
    ...
    reasoning_effort: str | None = Field(default=None)  # 源码版本的动态设置
    seed: int | None = Field(default=None)
    safety_settings: list[dict[str, str]] | None = Field(
        default=None,
        description='Safety settings for models that support them (like Mistral AI and Gemini)',
    )
    for_routing: bool = Field(default=False)  # 源码版本新功能
    
    # Custom proxy settings for internal network deployments (你的版本独有)
    use_token_auth: bool = Field(
        default=False,
        description='Use token authentication instead of API key (for custom proxy)',
    )
    proxy_headers: dict[str, str] | None = Field(
        default=None,
        description='Additional headers for custom proxy requests',
    )
    response_wrapper_field: str | None = Field(
        default=None,
        description='Field name that wraps the actual response (e.g., "data" for internal proxies)',
    )

    model_config = ConfigDict(extra='forbid')

    @model_validator(mode='after')
    def load_proxy_headers_from_env(self):
        """Load proxy_headers from environment variable if not set."""
        if self.proxy_headers is None:
            # Try to load from environment variable
            import json
            env_headers = os.environ.get('LLM_PROXY_HEADERS')
            if env_headers:
                try:
                    self.proxy_headers = json.loads(env_headers)
                    logger.info(f'[DEBUG] LLMConfig: 从环境变量加载proxy_headers: {self.proxy_headers}')
                except json.JSONDecodeError:
                    logger.warning(f'[DEBUG] LLMConfig: 无法解析环境变量LLM_PROXY_HEADERS: {env_headers}')
        return self
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        # Set reasoning_effort to 'high' by default for non-Gemini models (源码版本逻辑)
        ...
        # Set an API version by default for Azure models (源码版本新功能)
        ...
        # Set AWS credentials as environment variables for LiteLLM Bedrock (源码版本新功能)
