...
class Runtime(FileEditRuntimeMixin):
    ...
    def __init__(
        ...
        # Support external VSCode server in internal network environments
        is_internal_network = os.environ.get('LLM_USE_TOKEN_AUTH', '').lower() == 'true'
        #external_vscode_server = os.environ.get('EXTERNAL_VSCODE_SERVER_URL')
        #if not headless_mode and (not is_internal_network or external_vscode_server):
