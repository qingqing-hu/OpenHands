...
class Settings(BaseModel):
    ...
    llm_use_token_auth: bool = False
    llm_response_wrapper_field: str | None = None
    llm_proxy_headers: dict[str, str] | None = None
    ...
    def from_config() -> Settings | None:
        ...
        # Debug logging for custom proxy settings
        proxy_headers = getattr(llm_config, 'proxy_headers', None)
        from openhands.core.logger import openhands_logger as logger
        logger.info(f'[DEBUG] Settings.from_config: llm_config.proxy_headers = {proxy_headers}')
        logger.info(f'[DEBUG] Settings.from_config: llm_config.use_token_auth = {getattr(llm_config, "use_token_auth", False)}')
        ...
        settings = Settings(
            ...
            # Custom proxy settings from LLM config
            llm_use_token_auth=getattr(llm_config, 'use_token_auth', False),
            llm_response_wrapper_field=getattr(llm_config, 'response_wrapper_field', None),
            llm_proxy_headers=proxy_headers,
        )
