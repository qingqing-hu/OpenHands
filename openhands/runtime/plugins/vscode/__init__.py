...
class VSCodePlugin(Plugin):
    ...
    async def initialize(self, username: str, runtime_id: str = None) -> None:
        ...
        external_vscode_token = os.environ.get('EXTERNAL_VSCODE_TOKEN')

        if external_vscode_server:
            # Use external VSCode server
            logger.info(f'Using external VSCode server: {external_vscode_server}')
            try:
                # Extract port from URL if format is http://host:port
                from urllib.parse import urlparse
                parsed_url = urlparse(external_vscode_server)
                self.vscode_port = parsed_url.port or 80
            except Exception:
                self.vscode_port = 8080  # default port

            # Use provided token or generate a dummy one for external server
            self.vscode_connection_token = external_vscode_token or 'external-server'
            self.gateway_process = None  # No local process for external server

            logger.debug(f'External VSCode server configured: {external_vscode_server}')
            return
