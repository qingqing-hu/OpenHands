...
class ConversationMemory:
    ...
    def _process_action(
        ...
        if isinstance(
            ...
            # Ensure assistant messages are never empty to prevent API rejection (especially for Moonshot)
            content_text = assistant_msg.content
            if not content_text or not content_text.strip():
                # Provide default content for empty assistant messages to prevent API errors
                content_text = "I'll execute the requested action."
            ...
            # Ensure assistant messages are never empty for AgentFinishAction
            thought_text = action.thought
            if role == 'assistant' and (not thought_text or not thought_text.strip()):
                thought_text = "Task completed."
            
            return [
                Message(
                    role=role,  # type: ignore[arg-type]
                    content=[TextContent(text=thought_text or '')],
                )
            ]
        elif isinstance(action, MessageAction):
            role = 'user' if action.source == 'user' else 'assistant'
            # Ensure assistant MessageAction is never empty
            message_text = action.content or ''
            if role == 'assistant' and not message_text.strip():
                message_text = "I understand."
