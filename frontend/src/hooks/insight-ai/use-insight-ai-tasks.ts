...
export const insightAIKeys = {
  ...
  taskDetails: (taskId: string) =>
    [...insightAIKeys.tasks(), "detail", taskId] as const,
  taskLogs: (taskId: string, params?: any) =>
    [...insightAIKeys.tasks(), taskId, "logs", params] as const,
  taskFiles: (taskId: string, path?: string) =>
    [...insightAIKeys.tasks(), taskId, "files", path] as const,
...
export function useInsightAITaskLogs(
  taskId: string,
  params?: {
...
export function useInsightAIConversationHistory(
  ...
  return useQuery({
    ...
    queryFn: () =>
      InsightAIService.getConversationHistory(conversationId, params),
...
export function useCreateInsightAITask() {
  ...
  return useMutation({
    mutationFn: (taskData: CreateTaskRequest) =>
      InsightAIService.createTask(taskData),
    onSuccess: () => {
...
export function useCreateDataAnalysisTask() {
  ...
  return useMutation({
    mutationFn: (
      config: Parameters<typeof InsightAIService.createDataAnalysisTask>[0],
...
export function useCreateQueryAnalysisTask() {
  ...
  return useMutation({
    mutationFn: (
      config: Parameters<typeof InsightAIService.createQueryAnalysisTask>[0],
...
export function useInsightAIFileOperations(taskId: string) {
  ...
  const getFileContent = (filePath: string) =>
    InsightAIService.getFileContent(taskId, filePath);
...
export function useInsightAITaskStream(taskId: string, enabled = true) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const eventSource =
          await InsightAIService.subscribeToTaskUpdates(taskId);
        ...
        eventSource.onmessage = (event: MessageEvent) => {
          try {
            ...
            queryClient.setQueryData(
              insightAIKeys.taskDetails(taskId),
              (old: TaskDetails | undefined) =>
                ({
                  ...old,
                  ...data,
                }) as TaskDetails,
