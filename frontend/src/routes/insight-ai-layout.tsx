...
import { BiMessageRoundedAdd, BiRightIndent, BiLeftIndent, BiRefresh } from "react-icons/bi";
...
import { useInsightAITasks, useCreateInsightAITask, useDeleteInsightAITask } from "#/hooks/insight-ai/use-insight-ai-tasks";
...
export default function InsightAILayout() {
  ...
  const handleMouseMove = React.useCallback((e: MouseEvent) => {
    ...
    const constrainedWidth = Math.max(minRightWidth, Math.min(maxRightWidth, newWidth));
    ...
    <InsightAIProvider>
      <div className="insight-ai-layout bg-white h-screen">
        ...
        <div className="flex h-full overflow-hidden">
          ...
          <div className={`bg-gray-50 flex flex-col transition-all duration-300 flex-shrink-0 overflow-hidden ${sidebarCollapsed ? 'w-12' : 'w-72'}`}>
            <div className={`layout-header p-4 ${sidebarCollapsed ? 'border-b border-gray-200' : ''}`}>
              <div className="flex items-baseline justify-between">
                <div className="layout-title flex items-center gap-2">
                  {!sidebarCollapsed && (
                    <>
                      <h2 className="text-xl font-bold text-gray-900" style={{ lineHeight: '2rem' }}>对话管理</h2>
                      <button
                        onClick={async () => {
                      ...
                      >
                        <BiRefresh className={`w-4 h-4 text-gray-500 transition-transform duration-500 ${isRefreshing ? 'animate-spin' : ''}`} />
                      </button>
                    </>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-y-auto py-4 px-1.5 insight-ai-scrollbar">
                ...
                {tasksLoading ? (
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    <p className="text-red-800">Failed to load conversations: {tasksError.message}</p>
                  </div>
                ) : (
                  <InsightAITaskList
                    tasks={tasks}
                  ...
                  <div className="bg-white rounded-xl shadow-sm h-full flex flex-col overflow-hidden">
                    ...
                    <div className="bg-white px-6 py-3" style={{ height: '64px' }}>
                      <div className="flex items-center justify-end">
                        <button
                          ...
                          title={middlePanelExpanded ? "收起中间栏" : "展开中间栏"}
                        >
                          {middlePanelExpanded ? (
                    ...
                    <div className="flex-1 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <h3 className="text-xl font-semibold text-gray-900 mb-2">选择一个对话</h3>
                        <p className="text-gray-600">从左侧列表中选择一个对话来查看历史记录</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {!middlePanelExpanded && (
            ...
            >
              ...
              <div className="absolute -left-1 -right-1 top-0 bottom-0 bg-transparent"></div>
            </div>
          )}
              ...
              <div className="bg-white rounded-xl shadow-2xl border border-gray-200 p-6 max-w-sm w-full">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-base font-semibold text-gray-900 mb-1">新增对话</h3>
