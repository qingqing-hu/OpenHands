...
node_modules/
        ...
        if is_initial_prompt:
            ...
            # Prepare environment variables for Claude Code SDK
            claude_env = os.environ.copy()
            # Ensure ANTHROPIC_API_KEY is set from ANTHROPIC_AUTH_TOKEN if needed
            if 'ANTHROPIC_AUTH_TOKEN' in claude_env and 'ANTHROPIC_API_KEY' not in claude_env:
                claude_env['ANTHROPIC_API_KEY'] = claude_env['ANTHROPIC_AUTH_TOKEN']
                ui.info("Set ANTHROPIC_API_KEY from ANTHROPIC_AUTH_TOKEN", "Claude SDK")
            
            # Ensure ANTHROPIC_BASE_URL is passed through if available
            if 'ANTHROPIC_BASE_URL' in claude_env:
                ui.info(f"Using ANTHROPIC_BASE_URL: {claude_env['ANTHROPIC_BASE_URL']}", "Claude SDK")
            ...
            options = ClaudeCodeOptions(
                ...
                permission_mode="sandboxBashMode",
                model=cli_model,
