...
def init_git_repo(repo_path: str) -> None:
    ...
    if os.path.exists(os.path.join(repo_path, ".git")):
        print("Git repository already initialized by create-next-app")
        # Just ensure git config is set
        try:
            subprocess.run(["git", "config", "user.email", "claudable@example.com"], cwd=repo_path, check=True)
            subprocess.run(["git", "config", "user.name", "Claudable"], cwd=repo_path, check=True)
            print("Git user configuration updated")
        except Exception as e:
            print(f"Warning: Failed to configure git user: {e}")
    else:
